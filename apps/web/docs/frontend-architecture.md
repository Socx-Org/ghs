...
## Admin system settings (`ghs#157`)

**A frontend-only gap, not a backend one.** `GET /admin/settings` and the three `PUT` endpoints under `admin-settings.ts` already existed, fully correct and admin/super_admin-gated -- there was simply no page that called any of them, so `self_registration_enabled` (and `maintenance_mode`) could only ever be flipped by a manual SQL `UPDATE`. `AdminSettingsPage.tsx` (`/admin/settings`) is that missing page: five checkboxes (maintenance mode, self-registration, and the three notification toggles), each independently wired to its own `PUT` call.

**Each toggle is its own mutation, not one bulk "Save."** Matches the backend's own per-setting route shape (`PUT /admin/settings/maintenance-mode`, `.../self-registration-enabled`, `.../notifications/:type` are three separate endpoints, not one combined update), and gives each toggle its own immediate, honest success/error toast rather than a form-wide save state that couldn't distinguish which setting actually failed. The three notification toggles share one `useMutation`, disambiguated by comparing `.variables.type` -- the same established pattern as `AdminAccountsPage`'s own `statusMutation` (one mutation object, several independently-triggerable targets, per-target loading state read off `.variables`), not a new pattern invented here.

**No manual revert-on-error logic.** Every checkbox's `checked` value is read directly from `settingsQuery.data` -- never from local optimistic state. A successful mutation invalidates the query, pulling in the new confirmed value; a failed one simply never invalidates, so the query cache (and every checkbox reading from it) is exactly where it already was. Correct by construction, nothing to roll back.

**`notify_poll_interval_seconds` is deliberately not offered here** -- it exists in `system-settings.service.ts` but has no HTTP route at all yet (not even a `GET`), so exposing it would need new backend work first, out of this issue's scope.

**Turning self-registration on here genuinely unlocks the already-existing player-facing flow (`ghs#105`)**, not something newly built alongside it -- `LoginPage`'s "Create an account" link and `RegisterPage`'s own independent gate were both already correctly wired to `self_registration_enabled`; this issue's own screen is the only thing that was missing to ever set that value to `true` outside of raw SQL.
...
## Round entry: manual vs. CSV for hole scores (`ghs#160`)

**`RoundEntryPage` gains the same manual/CSV `ToggleGroup` pattern as `CreateCoursePage` (`ghs#155`)** -- "Enter manually" (today's per-`HoleEntryCard` form, unchanged) and "Load from CSV" (`components/domain/RoundHoleCsvImportForm.tsx`), which parses hole-level scores from an uploaded file and records them via the same `addHoleScore` calls each card already makes. Deliberately **hole-data only** (confirmed scope decision, `ghs#160`'s own text) -- course/tee configuration/player/date/tournament/9-hole are not part of this CSV at all; they're already fixed by the time a round reaches this screen. This sidesteps the harder problem a fuller "whole round from file" import would need: matching a source file's own `course_id`/`tee_configuration_id`/`player_id` against this app's real records, since a round can only ever reference ones that already exist (unlike `ghs#155`'s course import, which creates new rows).

**Per-hole strokes/putts validation is now shared, not duplicated, between the manual card and the CSV importer** -- `requiredStrokes`/`optionalNonNegative` moved out of `HoleEntryCard.tsx` into `lib/hole-score-schema.ts` (same "component file exporting a schema alongside its component breaks Fast Refresh" reasoning already applied to `TeeConfigurationForm.tsx`/`lib/tee-configuration-schema.ts` in `ghs#155`'s own review). `gir`/`fairwayResult`/`inSand`/`penalties`, however, get a **separate** CSV-specific schema (`csvHoleRowSchema`) rather than a relaxation of `HoleEntryCard`'s own `holeFormSchema` -- those three fields are always-defined in the manual form (a checkbox/number input always has a concrete value to submit), but genuinely optional in the CSV format (a blank cell means "leave this field alone," matching `addHoleScore`'s own upsert semantics). Widening `holeFormSchema` itself to match would have silently changed the manual form's own existing behaviour (today, clearing the penalties field and saving records `0`, not "leave whatever's already there alone") -- out of this issue's scope, so the two schemas stay genuinely separate where their real semantics differ, and share the primitives where they don't.

**`fairway_hit` is `TRUE`/`LEFT`/`RIGHT`, not a boolean** -- a deliberate clarification from the platform owner during scoping, distinct from what the actual sample round-export provided at the time literally contained (a plain `true`/blank column with no miss-direction captured at all). Maps directly onto `fairwayResult`'s own three-value domain (`"hit"` / `"missed_left"` / `"missed_right"`), matching what a `HoleEntryCard`'s own `ToggleGroup` already lets a player record manually.

**Each hole is its own independent `addHoleScore` call, attempted sequentially, not `Promise.all`** -- deliberately not concurrent, to avoid any unverified risk of racing writes to the same round's hole scores; for a 9-or-18-row file the sequential cost is imperceptible. A per-hole failure is reported against that specific hole (`Failed`, with the real server error) and never aborts the rest, matching this issue's own explicit requirement -- the preview list shows every hole's real status (`Will import` -> `Imported`/`Failed`, or `Skipped` + why for a client-side validation failure) rather than treating the whole file as one atomic operation.
...
**Pagination (`ghs#138`) is always on, not opt-in like search/filter** -- unlike `getSearchText`/`filters`, which need a screen to designate its own searchable fields, slicing an already-known array into pages needs no per-screen configuration, so `ListView` just does it unconditionally with a `pageSize` prop (default 10, overridable per screen). It applies to the already-filtered result, deliberately sequenced after `#137` so a search/filter narrows first and pagination slices what's left, not the raw array. The `Pagination` control (`Previous`/`Page X of Y · N results`/`Next`, `<nav aria-label="Pagination">`) renders nothing at all once a result set already fits on one page -- real screens with small datasets (e.g. most of `AdminAccountsPage` today) never show it, only screens/searches whose result exceeds `pageSize` do.

**The current page resets to 1 whenever the search query or a filter value changes** -- adjusted during render (React's own documented pattern for "reset state when an input changes": track the previous search/filter key in state, compare on every render, `setState` immediately if it differs), not inside a `useEffect` -- calling `setState` synchronously inside an effect triggers an avoidable extra cascading render, flagged by this codebase's own `react-hooks/set-state-in-effect` lint rule. The current page is additionally clamped to the real last page on every render regardless of *why* the filtered result shrank, not only in response to a search/filter edit.

**No sorting or virtualization built into `ListView`** -- sorting remains genuinely out of scope, per the design doc's own "do not invent behaviour without backend support" rule for anything beyond what a consuming screen's endpoint already returns (still true post-`#138`: pagination only reorders which slice of the already-ordered array is visible, it doesn't change the order itself). No virtualization either -- not justified until a real screen's dataset size demands it.

## Course creation: manual entry vs. CSV import (`ghs#110`, `ghs#155`)

**`CreateCoursePage` offers two entry modes via a `ToggleGroup`** -- "Enter manually" (`ghs#110`'s original form, unchanged: name/city/country only, no tee-configuration fields) and "Load from CSV" (`ghs#155`, `components/domain/CourseCsvImportForm.tsx`), which parses a course and one or more tee configurations from an uploaded file and submits both in the same `POST /courses` call the manual form's own `createCourse()` wrapper already used -- `CreateCourseRequest` (`lib/api.ts`) just gained an optional `teeConfigurations?: TeeConfigurationInput[]` the manual form still never populates, by `ghs#110`'s own design (tee configurations are still added afterward via `CourseDetailPage`, `ghs#112`, when entering manually).

**One validation schema, not two.** `teeConfigurationSchema`/`toTeeConfigurationInput` (originally private to `TeeConfigurationForm.tsx`) now live in `lib/tee-configuration-schema.ts` -- both the manual `TeeConfigurationForm` and the CSV importer (`lib/course-csv.ts`) validate every tee configuration through the exact same rules, so a row that wouldn't pass manual entry doesn't pass CSV import either, and neither can silently drift from the other. Kept out of `TeeConfigurationForm.tsx` itself (rather than just exported from there) because a component file exporting a plain schema/function alongside its component breaks Fast Refresh (`react-refresh/only-export-components`) -- same reasoning as `lib/domain-labels.ts`'s extraction from `RoleBadge`/`AccountStatusBadge`/`RoundStatusBadge`.

**CSV parsing/grouping (`lib/course-csv.ts`) is framework-free and directly unit-tested** against real sample data, not React-rendered. One row per hole; rows group into one tee configuration per distinct `configuration_id` (the source file's own id, read only to group rows -- never reused as this app's own id, same as every other creation path); `course_name`/`course_city`/`course_country` are read once from the first row, not re-validated per row. A tee configuration's `name` combines `configuration_name` + `tee_colour`, but only when the colour isn't already part of the name (real source data does both: one sample's `configuration_name` is plain, "Members"; another's already reads "Golf Costa Brava - Verde - Blue Tee (Male)").

**A tee configuration that fails validation is skipped, not the whole file.** Missing `course_rating`/`slope_rating`, an unrecognised hole count, a duplicate or missing hole number, or any per-hole error each produce a `{ valid: false, reason }` outcome for that one tee configuration -- every other valid one in the same file still imports. `CourseCsvImportForm` shows every outcome (name + "Will import"/"Skipped" + reason) as a preview *before* any submit happens, so an admin sees exactly what will and won't be created, never discovering a partial import only after the fact. File-level failures (empty file, missing required columns, zero data rows, a row missing `configuration_id` entirely) throw `CourseCsvParseError` instead, shown as a single clear `Alert`, blocking submission entirely -- distinct from a per-tee-configuration skip, since there's nothing coherent to preview at all.
...
## Icon + label buttons (`ghs#134`)

`Button` gained an `icon?: ReactNode` prop rendered immediately before `children`. The icon is **replaced by**, not shown alongside, the loading spinner while `isLoading` is true -- one leading glyph slot, not two competing ones. `Button` doesn't size, colour, or `aria-hide` the icon itself; the caller passes an already-styled element (`icon={<Trash2 aria-hidden="true" className="h-4 w-4" />}`), the same call-site-sizes-its-own-icon convention `Sidebar`/`AccountMenu`/`ThemeToggle` already used before this prop existed. `h-4 w-4` is the convention for a `Button`'s leading icon specifically (matching `AccountMenu`'s own icon+label menu items, the closest existing precedent to a labelled icon control) -- distinct from the `h-5 w-5` used for standalone/nav icons elsewhere. This is additive to, not a replacement for, the existing icon-only pattern (icon passed as `children`, `aria-label` required) -- the two are orthogonal and both still work.

Applied deliberately, not mechanically, per the issue's own instruction not to add an icon to every button matching a given label:

- **Back navigation**: extracted into a shared `BackButton` (`components/BackButton.tsx`, wraps `Button` with a fixed `ArrowLeft` icon, `variant="ghost" size="sm"`), replacing ten hand-duplicated `<Button variant="ghost" size="sm">← Back</Button>`-shaped call sites. A real `ArrowLeft` icon replaces the old em-dash-arrow text glyph -- a genuine accessibility fix, not just visual, since "←" isn't reliably announced as "back" by assistive tech. Deliberately does not cover the four unauthenticated auth-flow pages' own "← Back to sign in" links (`RegisterPage`/`ForgotPasswordPage`/`ResetPasswordPage`/`ActivationPage`) -- those are plain inline `Link` text under a form, not a `Button`, a different UI pattern this issue's "icons in buttons" scope doesn't reach.
- **Delete**: every genuine delete action (row-level triggers and their modal confirm buttons alike) gets `Trash2` -- `AdminAccountsPage`, `AdminRoundsListPage`, `MyRoundsPage`, `AdminRoundReviewPage`, `CourseDetailPage`. Explicitly excludes `AdminRoundReviewPage`'s "Reject"/"Reject round" buttons (`destructive` variant, but rejecting isn't deleting).
- **Create**: `Plus` is used only on header-level buttons that navigate *to* a create screen (`AdminAccountsPage`'s "Create account", `CourseListPage`'s "Create course", `CourseDetailPage`'s "Add tee configuration") -- not on the form-submit buttons with the same visible text inside the create forms themselves (`RegisterPage`, `AdminCreateUserPage`, `CreateCoursePage`), which are judged a different action class (submitting a filled form vs. starting one).
- **Edit**: not one of the issue's own named examples, but added (`Pencil`) wherever an Edit button sits directly beside a Delete button already getting `Trash2` in the same row (`MyRoundsPage`, `CourseDetailPage`'s tee-configuration rows) -- leaving Edit as bare text next to an icon+text Delete in the same row would itself be the inconsistency the issue asks to avoid.
- **Sign out**: `AccountMenu`'s "Sign out" (`LogOut`) and "Profile" (`User`) already combined icon+label before this issue -- confirmed unchanged, no action needed; they're plain `<button>`/`<Link>` menu items, not `Button` instances, so the new prop doesn't apply there anyway.

**Follow-up refinement pass**, widening the audit beyond the issue's own named examples:

- **`ToggleGroup` gained an `iconOnly` prop** (plus a per-option `icon`), used to compact `ListView`'s own Table/Grid view switch down to icon-only. The option's `label` text is never removed from the DOM -- just visually hidden (`sr-only`) -- so it stays each radio's accessible name via the same implicit `<label>`-wrapping the visible-text case already relies on; no parallel `aria-label` to keep in sync. Every other `ToggleGroup` consumer (`HoleEntryCard`'s GIR toggle, `RoundEntryPage`/`CreateCoursePage`'s Manual/CSV toggle) is untouched -- `iconOnly` is opt-in per instance, not a global behaviour change.
- **`ListView` row actions are icon-only**, not just their Table/Grid switch: `AdminAccountsPage` (Disable/Enable -- `Ban`/`CircleCheck` -- and Delete), `AdminRoundsListPage` (Delete), `MyRoundsPage` (Edit/Delete). Built on the `icon` prop with `children` omitted (not the icon-as-`children` pattern `ThemeToggle` uses) -- omitting `children` is what makes `Button`'s own `isIconOnly` check true, which is what applies its square `w-11 px-0` sizing. Each button's `aria-label` names the row explicitly (e.g. `` `Delete ${item.email}` ``, `` `Edit round at ${item.courseName}` ``) rather than leaving several identically-labelled icon buttons on one page for assistive tech to disambiguate by position alone. `CourseDetailPage`'s tee-configuration rows are a plain `<ul>`/`<li>`, not a `ListView`, so its Edit/Delete keep their visible text.
- **Pagination's Previous/Next** (`ListView`'s own `Pagination`) gained `ChevronLeft`/`ChevronRight` -- kept alongside their text, not icon-only, since this is a nav control read in a list, not a row action. `Next` composes its icon into `children` after the text (`Button` has no trailing-icon slot; the `icon` prop always renders first) rather than extending `Button`'s API for one call site.
- **Save**: `CourseDetailPage`'s course-edit "Save changes" and `TeeConfigurationForm`'s own submit gain `Save`, but only in edit mode (`submitLabel === "Save changes"`, i.e. `initialValues` provided) -- its create-mode "Add tee configuration" submit stays icon-free, consistent with every other form-submit "Create X" button below.
- **Approve/Reject** (`AdminRoundReviewPage`): `Check`/`X` on the trigger buttons; the Reject confirmation modal's own "Reject round" button gets the same `X`, matching the trigger+confirm icon consistency already established for Delete.
- **Cancel, in delete-confirmation modals specifically**: `X`, added to every Delete/`Delete round` modal's Cancel button (`AdminAccountsPage`, `AdminRoundsListPage`, `MyRoundsPage`, `AdminRoundReviewPage`'s delete-round modal, `CourseDetailPage`'s tee-configuration and course delete modals). Deliberately not every Cancel button app-wide -- `AdminRoundReviewPage`'s Reject-modal Cancel and `TeeConfigurationForm`'s own create/edit-form Cancel are outside a *delete* confirmation and stay text-only.
- **Auth-flow primary actions**: `LogIn` (`LoginPage`'s "Sign in"), `Send` (`ForgotPasswordPage`'s "Send reset link"), `UserPlus` (`RegisterPage`'s "Create account" submit -- a deliberate, explicit exception to the create-form-submit rule above, made for this specific button). `LoginPage`'s "Forgot my password?" is a `Link`, not a `Button`, so its `KeyRound` icon is composed inline (`<Link><KeyRound />Forgot my password?</Link>`) rather than via the `icon` prop.
- **New round**: `Plus`, on both `MyRoundsPage`'s and `PlayerDashboardPage`'s "New round" buttons -- these navigate to a create screen, the same class of button as `CourseListPage`'s "Create course" above, not a form submit.
- **Change password**: `KeyRound`, on `ProfilePage`'s submit button.
